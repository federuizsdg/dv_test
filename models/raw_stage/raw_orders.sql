...
FROM {{ source('snowflake_sample', 'ORDERS') }} AS b
LEFT JOIN {{ source('snowflake_sample', 'LINEITEM') }} AS a
    ON a.L_ORDERKEY = b.O_ORDERKEY
LEFT JOIN {{ source('snowflake_sample', 'CUSTOMER') }} AS c
...
LEFT JOIN {{ source('snowflake_sample', 'PART') }} AS g
    ON a.L_PARTKEY = g.P_PARTKEY
LEFT JOIN {{ source('snowflake_sample', 'SUPPLIER') }} AS h
    ON a.L_SUPPKEY = h.S_SUPPKEY
LEFT JOIN {{ source('snowflake_sample', 'NATION') }} AS j
    ON h.S_NATIONKEY = j.N_NATIONKEY
LEFT JOIN {{ source('snowflake_sample', 'REGION') }} AS k
