{% macro generate_hub (metadata_dict) %}
    ...
    {%- set row_rank_template.value = row_rank_template.value ~ '  SELECT <SOURCEALIAS>.<PK>, <NK>, <SOURCEALIAS>.<LDTS>, <SOURCEALIAS>.<SOURCE>,'~ '\n' -%}
    ...
    {%- set row_rank_template.value = row_rank_template.value ~ '  WHERE <SOURCEALIAS>.<PK> IS NOT NULL'~ '\n' -%}
    ...
    {%- set row_rank_union_template.value = row_rank_union_template.value ~ '    WHERE ru.<PK> IS NOT NULL' ~ '\n' -%}
    ...
    {%- set records_to_insert_template.value = records_to_insert_template.value ~ 'SELECT <SOURCEALIAS>.<PK>, <NK>, a.<LDTS>, a.<SOURCE>' ~ '\n' -%}
    ...
    {%- for model_name in models_to_link -%}
        ...
        {%- set row_rank_item.value = row_rank_item.value.replace('<PK>', metadata_dict['src_pk'] ) -%}
        {%- set row_rank_item.value = row_rank_item.value.replace('<NK>', generate_source_fields_list( metadata_dict['src_nk'] , 'rr' ) ) -%}
    ...
    {%- if models_to_link|length > 1 -%}
      ...
      {%- set row_rank_union.value = row_rank_union.value.replace('<PK>', metadata_dict['src_pk'] ) -%}
    ...
    {%- set records_to_insert.value = records_to_insert.value.replace('<PK>', metadata_dict['src_pk'] ) -%}
    {%- set records_to_insert.value = records_to_insert.value.replace('<NK>', generate_source_fields_list( metadata_dict['src_nk'] , 'a') ) -%}
